...
def get_audio_duration(audio_path: str, timeout: int = 5) -> float:
    ...
    if sf is None:
        missing_backends.append("soundfile")
    if torchaudio is None:
        missing_backends.append("torchaudio")
    if missing_backends:
        missing = ", ".join(missing_backends)
        raise RuntimeError(
            f"failed to get duration for {audio_path}: ffprobe unavailable or failed, and {missing} is not installed"
        ) from last_error
    raise RuntimeError(f"failed to get duration for {audio_path}") from last_error
